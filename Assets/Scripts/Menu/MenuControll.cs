...
public class MenuControll : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        pig = GameObject.Find("BackgroundMenu").transform.FindChild("back1").FindChild("heo");
        LeanTween.moveX(pig.gameObject, -2.5f, 5).setOnCompleteOnRepeat(true).setOnComplete(() =>
        {
            ...
            if (i == 1)
            {
                pig.FindChild("lon").GetComponent<Animator>().SetTrigger("nhay");
            }
        }).setLoopPingPong();
        //audio
        ...
        isShowLogin = false;
        dialogLogin = transform.parent.FindChild("DialogLogin");
        ...
        lbTouchStart = transform.FindChild("Label").GetComponent<UILabel>();
        lbTouchStart.gameObject.SetActive(false);
        // Khở tạo ngôn ngữ cho HungBV
        SetLaguage();
        transform.FindChild("Label").GetComponent<UILabel>().text = MissionControl.Language["TOUCH_START"];
        if ("Vietnamese".Equals(VariableSystem.language))
        {
    ...
    {
        GameObject.Find("BackgroundMenu").transform.FindChild("back1").FindChild("Xoay").gameObject.SetActive(true);
    }
}
