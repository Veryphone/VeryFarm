...
public class CustomerScript : MonoBehaviour
{
    ...
    public void CreateCustomer(int indexR, int indexC, int typeCus, int indexP, bool isNewCus = false)//add Object
    {
        ...
        tempTransform = cusObjects[indexR * 5 + indexC].transform.FindChild("Customer");//destroy previous object
        ...
        if (indexP != -1)
        {
            ...
            {
                sprite.GetComponent<Renderer>().sortingLayerName = "14";
            }
        }
        if (indexC == 0)
        {
    ...
    {
        //thay đổi hàng chờ
        tempTransform = cusObjects[row * 5].transform.FindChild("Customer");//destroy object in colum 0
        if (tempTransform != null)
        {
            GameObject.Destroy(tempTransform.gameObject);
        }
        for (i = 1; i < 5; i++)
        {
            tempTransform = cusObjects[row * 5 + i].transform.FindChild("Customer");
            ...
            foreach (SpriteRenderer sprite in sprites)
            {
				sprite.GetComponent<Renderer>().sortingOrder =( 5 * (2 - row) + i - 1);
            }
        }
    ...
    internal void deleteColum(int indexRowDelete)//delete one row when you fire one staff
    {
        for (i = 0; i < 5; i++)
        {
            ...
            {
                GameObject.Destroy(cusObjects[indexRowDelete * 5 + i].transform.FindChild("Customer").gameObject);
