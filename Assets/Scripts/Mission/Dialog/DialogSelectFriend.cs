...
public class DialogSelectFriend : DialogAbs
{
    ...
    void Awake()
    {
        ...
        gridView = transform.FindChild("ScrollView").FindChild("Grid");
        buttonSendLife = transform.FindChild("SendLife").GetComponent<UIButton>();
        buttonSendInvite = transform.FindChild("SendInvite").GetComponent<UIButton>();
        dialogLoading = transform.parent.FindChild("DialogLoading");
        checkBoxSelectAll = transform.FindChild("SelectAll").FindChild("CheckboxSelectAll");
    ...
    public void ShowDialogInviteFriend()
    {
        ...
        this.transform.FindChild("SendLife").gameObject.SetActive(false);
        this.transform.FindChild("SendInvite").gameObject.SetActive(true);
        this.transform.FindChild("SendAskLife").gameObject.SetActive(false);
        this.transform.FindChild("ForeGround").FindChild("Title").gameObject.GetComponent<UILabel>().text = MissionControl.Language["Invite_friend_detail"];
        ...
        this.transform.FindChild("SendInvite").FindChild("Label").GetComponent<UILabel>().text = MissionControl.Language["Send"];
    ...
    public void SelectAllButton()
    {
        ...
        if (isCheck)
        {
            for (int i = 0; i < arrItem.Count; i++)
            {
                arrItem[i].FindChild("Checkbox").gameObject.GetComponent<UIToggle>().value = true;
            }
        }
        else
        {
            if (!unselectFriend)
            {
                for (int i = 0; i < arrItem.Count; i++)
                {
                    arrItem[i].FindChild("Checkbox").gameObject.GetComponent<UIToggle>().value = false;
                }
            }
        }
        unselectFriend = false;
    ...
    public void ShowDialogAskFriend()
    {
        ...
        this.transform.FindChild("SendLife").gameObject.SetActive(false);
        this.transform.FindChild("SendInvite").gameObject.SetActive(false);
        this.transform.FindChild("SendAskLife").gameObject.SetActive(true);
        this.transform.FindChild("ForeGround").FindChild("Title").gameObject.GetComponent<UILabel>().text = MissionControl.Language["Ask_friend_detail"];
        this.transform.FindChild("ForeGround").FindChild("Logo").FindChild("name").GetComponent<UILabel>().text = MissionControl.Language["Ask_friend"];
        this.transform.FindChild("SelectAll").FindChild("Label").GetComponent<UILabel>().text = MissionControl.Language["Select_all"];
        this.transform.FindChild("SendAskLife").FindChild("Label").GetComponent<UILabel>().text = MissionControl.Language["Send"];
        gridView.transform.parent.GetComponent<UIPanel>().clipOffset = new Vector2();
        gridView.transform.parent.localPosition = new Vector2();
        gridView.parent.GetComponent<UIScrollView>().ResetPosition();
        DFB.FBGetUserFriend(100, result =>
